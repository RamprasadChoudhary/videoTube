...
const getChannelStats = asyncHandler(async(req,res) => {
    ...
    const totalVideoLikes = await Like.aggregate(
        [
            ...
            {
                $match:{
                    'likedVideo.owner': new mongoose.Types.ObjectId(userId)
                }
            },
            {
                $group:{
    ...
    const totalCommentLikes = await Like.aggregate([
        ...
        {
            $match:{
                'commentsLiks.owner':new mongoose.Types.ObjectId(userId)
            }
        }
    ])
    ...
    const totalikes = totalTweetsLikes.length + totalCommentLikes.length + totalVideoLikes[0].totalLikes;
